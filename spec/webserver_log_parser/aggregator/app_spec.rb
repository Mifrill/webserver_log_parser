...
describe WebserverLogParser::Aggregator::App do
  ...
  describe '#call' do
    it 'yields array with views' do
      ...
      expect(scope).to receive(:average_page_views).once.with(no_args).and_return(scope)
      expect(scope).to receive(:order_by_most_views).once.with(no_args).and_return(scope)
      expect(scope).to receive(:to_a).once.with(no_args).and_return(['average_page_views-rows'])
      expect { |block| aggregator.call(scope, &block) }
        .to yield_with_args([['total_pages_views-rows'], ['order_by_most_views-rows'], ['average_page_views-rows']])
    end
  end
end
