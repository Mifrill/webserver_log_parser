...
describe WebserverLogParser::Aggregator::App do
  ...
  describe '#call' do
    it 'yields array with views' do
      allow(scope).to receive(:total_pages_views).once.with(no_args).and_return(scope)
      ...
      allow(scope).to receive(:unique_pages_views).once.with(no_args).and_return(scope)
      expect(scope).to receive(:order_by_most_views).once.with(no_args).and_return(scope)
      ...
      expect { |block| aggregator.call(scope, &block) }
        .to yield_with_args([['total_pages_views-rows'], ['order_by_most_views-rows']])
    end
  end
end
